...
def find_nearest_od_time(ld_time, od):
    od_cols = ['pose.pose.position.x','pose.pose.position.y', 'pose.pose.position.z',
       'pose.pose.orientation.x', 'pose.pose.orientation.y',
       'pose.pose.orientation.z', 'pose.pose.orientation.w']
    nearest_index = (od['Time'] - ld_time).abs().idxmin()
    return od.loc[nearest_index, od_cols]
...
if __name__ == "__main__":
    ...
    ld = ld['Time'].apply(lambda x: find_nearest_od_time(x, od))
